...
#####in total (QC-passed reads + QC-failed reads)
    -total fragments before mapping
#####secondary
#####supplementary
#####duplicates
...
#####read1
  -how many "read 1" fragments mapped
#####read2
  -how many "read 2" fragments mapped
...
p<- (ggplot(comb,aes(x=as.character(comb$Sample), y=as.numeric(as.character(comb$Value)), fill=as.character(comb$Metric)))+ 
        ...
        ylim(0, 65000000)+
        ylab("Absolute Fragment Count")+
        xlab("Sample (not ordered)")+
        ggtitle("Absolute Fragments, Mapped Frag, and Unique Frag", subtitle = "(reads not marked as duplicated by markdup)") +
...
f<- (ggplot(comb,aes(x=as.character(comb$Sample), y=as.numeric(as.character(comb$Value)), fill=as.character(comb$Metric)))+ 
        ...
        ylab("Absolute Fragment Count")+
        xlab("Sample (not ordered)")+
        ggtitle("Percent Mapped Fragments and Unique Fragments", subtitle = "(frags not marked as duplicated by markdup)")
